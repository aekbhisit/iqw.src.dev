...
function setCategoryDuplicate(id){
	...
	$.get(url, function(data){
		if(data=='isAdmin'){
					alert('ไม่สามารถคัดลอกหมวดหมู่นี้ได้เนื่องจากเป็นหมวดหมู่หลัก!');
					$.jGrowl("แจ้งเตือน ! <br> คัดลอกหมวดหมู่นี้ไม่ได้", {position: "bottom-right"});
		}else{
			reloadTableData(oTable);
...
function setCategoryDelete(id){
	if(confirm('ยืนยันลบหมวดหมู่')){
		...
		var url = "../../app/index.php?module="+modules+"&task=setCategoryDelete&id="+id+"&d"+d.getTime() ;
		$.get(url, function(data){
			if(data=='haschild'||data=='hasitem'||data=='isAdmin'){
				if(data=='haschild'){
					alert('ไม่สามารถลบหมวดหมู่นี้ได้เนื่องจากมีหมวดหมู่ย่อยอยู่ !');
					$.jGrowl("แจ้งเตือน ! <br> ต้องลบหมวดหมู่ย่อยและข้อมูลภายในหมวดหมู่นี้ก่อน", {position: "bottom-right"});
				}
				if(data=='hasitem'){
					alert('ไม่สามารถลบหมวดหมู่นี้ได้เนื่องจากมีข้อมูลอยู่ !');
					$.jGrowl("แจ้งเตือน ! <br> ต้องลบข้อมูลในหมวดหมู่นี้ก่อน", {position: "bottom-right"});
				}
				if(data=='isAdmin'){
					alert('ไม่สามารถลบหมวดหมู่นี้ได้เนื่องจากเป็นหมวดหมู่หลัก!');
					$.jGrowl("แจ้งเตือน ! <br> ลบหมวดหมู่นี้ไม่ได้", {position: "bottom-right"});
				}
			}else{
				reloadTableData(oTable);
