...
function categoryFormInit(){
	...
	var url = "../../app/index.php?module=users&task=categoryFormInit&"+request+"&d"+d.getTime() ;
...
function loadNowCategories(selected){
	var d = new Date();
	var url = "../../app/index.php?module=users&task=loadCategories&d"+d.getTime() ;
	$.getJSON(url,function(data){
		//alert(data);
...
function setSaveCategories(){
	var d = new Date();	
	var url = "../../app/index.php?module=users&task=saveCategory&d"+d.getTime() ;
	//alert(url);
	$.ajax({
		  type: 'POST', 
		  url: url, 
