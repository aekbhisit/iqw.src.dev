...
function formInit(){
	...
	var url = "../../app/index.php?module=members&task=loadUsersAddress&"+request+"&d"+d.getTime() ;
...
function setSaveUsersAddress(){
	var d = new Date();	
	var request = window.location.search.replace('?','') ;
	var url = "../../app/index.php?module=members&task=saveUsersAddress&"+request+"&d"+d.getTime() ;
	$.ajax({
		  type: 'POST', 
		  url: url, 
...
function setUsersAddressDelete(){
	...
	if(id!=''&&id!='add'){
		if(confirm('ยืนยันลบที่อยู่สมาชิก')){
			var d= new Date();
			var url = "../../app/index.php?module=members&task=setUsersAddressDelete&id="+id+"&d"+d.getTime() ;
...
function setUserAddressStatus(id,status){
	var d= new Date();
	var url = "../../app/index.php?module=members&task=setUsersAddressStatus&status="+status+"&id="+id+"&d"+d.getTime() ;
	$.get(url, function(data){
