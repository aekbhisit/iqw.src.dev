...
function categoryFormInit(){
	...
	var url = "../../app/index.php?module=members&task=categoryFormInit&"+request+"&d"+d.getTime() ;
...
function loadNowCategories(selected){
	var d = new Date();
	var url = "../../app/index.php?module=members&task=loadCategories&d"+d.getTime() ;
	$.getJSON(url,function(data){
		//alert(data);
...
function setSaveCategories(){
	var d = new Date();	
	var url = "../../app/index.php?module=members&task=saveCategory&d"+d.getTime() ;
	//alert(url);
	$.ajax({
		  type: 'POST', 
		  url: url, 
