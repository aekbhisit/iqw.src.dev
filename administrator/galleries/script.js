...
function setDeleteData(id,confirmed,length,index){
	...
	if(confirmed){
		$.get(url,function(data){
			if(length==(index+1)){ 
				reloadTableData(oTable);
				$.jGrowl("แจ้งเตือน ! <br> ลบรายการที่เลือกเสร็จ", {position: "bottom-right"} );
			}
		});
	}else{
		if(confirm('ยืนยันลบข้อมูลที่เลือก !')){
...
function setGalleriesStatus(id,status){
	var d = new Date();
	var url = "../../app/index.php?module="+modules+"&task=setGalleriesStatus&status="+status+"&id="+id+"&d"+d.getTime();
	$.get(url, function(data){
		reloadTableData(oTable);
		$.jGrowl("แจ้งเตือน ! <br>เปลี่ยนสถานะอัลบั้มสำเร็จ", {position: "bottom-right"} );
	});
}
function setGalleriesEdit(id){
...
function setGalleriesDelete(id){
	if(confirm('ยืนยันลบหมวดหมู่')){
		var d = new Date();
		var url = "../../app/index.php?module="+modules+"&task=setDelete&id="+id+"&d"+d.getTime();
		$.get(url, function(data){
			reloadTableData(oTable);
			$.jGrowl("แจ้งเตือน ! <br> ลบอัลบั้มสำเร็จ", {position: "bottom-right"} );
		});
	}
}
function setGalleriesMove(id,position){
	var d = new Date();
