...
function setCategoryStatusIndex(id,status){
	var d= new Date();
	var url = "../../app/index.php?module="+modules+"&task=setCategoryStatusIndex&status="+status+"&id="+id+"&d"+d.getTime() ;
	$.get(url, function(data){
		reloadTableData(oTable);
		$.jGrowl("แจ้งเตือน ! <br>เปลี่ยนสถานะหน้าแรกสำเร็จ", {position: "bottom-right"});
		createRouteFile();
	});
}
...
function createRouteFile(){
	var d= new Date();
		...
		$.get(url, function(data){
				reloadTableData(oTable);
				$.jGrowl("แจ้งเตือน ! <br> สร้าง File route.php สำเร็จ", {position: "bottom-right"});
		});
}