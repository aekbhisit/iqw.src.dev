...
(function($) {
	$(document).ready(function(e) {
		...
		oTable = $("table#da-ex-datatable-numberpaging").dataTable({
        	...
        	"bServerSide": true,
			"bStateSave": true,
        	"sAjaxSource": "../../app/index.php?module="+modules+"&task=getAttrData",
...
function setDeleteData(id,confirmed,length,index){
	var d = new Date() ;
	var  url = "../../app/index.php?module="+modules+"&task=setAttrDelete&id="+id+"&d"+d.getTime() ;
	//alert(url);
	if(confirmed){
...
function setStatus(id,status){
	var d= new Date();
	var url = "../../app/index.php?module="+modules+"&task=setAttrStatus&status="+status+"&id="+id+"&d"+d.getTime() ;
	$.get(url, function(data){
		reloadTableData(oTable);
...
function setEdit(id){
	var url ="attr-form.php?mode=edit&id="+id;
	window.location.replace(url);
}

function setDuplicate(id){
	var d= new Date();
	var url = "../../app/index.php?module="+modules+"&task=duplicateAttr&id="+id+"&d"+d.getTime() ;
	$.get(url, function(data){
		reloadTableData(oTable);
...
function setTranslate(id){
	var url ="attr-form-translate.php?mode=translate&id="+id;
	window.location.replace(url);
}
...
function setMove(id,position){
		var d= new Date();
		var url = "../../app/index.php?module="+modules+"&task=setAttrMove&type="+position+"&id="+id+"&d"+d.getTime() ;
		$.get(url, function(data){
				//alert(data);
				reloadTableData(oTable);
...
function LoadCategoryFilter(){
	var d= new Date();
	var url = "../../app/index.php?module="+modules+"&task=loadCategoriesAttrFilter&d"+d.getTime() ;
	$.get(url, function(data){
...
function sortable(status){
	if(status){
		...
		$( "#sortable" ).bind( "sortstop", function(event, ui) {
			...
			var d= new Date();
			var url = "../../app/index.php?module="+modules+"&task=reorderAttrData&sort="+sort_list+"&id="+id_list+"&d"+d.getTime() ;
			$.get(url, function(data){
				reloadTableData(oTable);
...
function switchDataOrder(id){
	 ...
	 var d= new Date();
	var url = "../../app/index.php?module="+modules+"&task=switchAttrOrder&id="+id+"&sort="+sort_val+"&d"+d.getTime() ;
	 $.get(url, function(data){
			reloadTableData(oTable);
...
function setReorderAttr(){
	var sort_rule =oTableData.aaSorting+'';
	var sort_split = sort_rule.split(',') ;
	var column = sort_split[0] ;
	var direction = sort_split[1] ;
	var d= new Date();
	var url = "../../app/index.php?module="+modules+"&task=setReorderAttr&column="+column+"&direction="+direction+"&d"+d.getTime() ;
