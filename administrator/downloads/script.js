...
function setDeleteData(id,confirmed,length,index){
	...
	if(confirmed){
		...
		if(confirm('ยืนยันลบข้อมูลที่เลือก !')){
			$.get(url,function(data){
				reloadTableData(oTable);
				$.jGrowl("แจ้งเตือน ! <br> ลบรายการเสร็จ", {position: "bottom-right"});
			});
		}
	}
}
...
function setStatus(id,status){
	...
	$.get(url, function(data){
		reloadTableData(oTable);
		$.jGrowl("แจ้งเตือน ! <br>เปลี่ยนสถานะสำเร็จ", {position: "bottom-right"});
	});
}
...
function setDuplicate(id){
	...
	$.get(url, function(data){
		reloadTableData(oTable);
		$.jGrowl("แจ้งเตือน ! <br>คัดลอกสำเร็จ", {position: "bottom-right"});
	});
}
