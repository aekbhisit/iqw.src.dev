...
if(isset($_GET['task'])){
	...
	switch($task){
		...
		case 'getCommentsData':
			...
			$orderby = '';
			$search = '';
			$iDisplayLength = $oComments->setInt($_GET['iDisplayLength']);
			$iDisplayStart = $oComments->setInt($_GET['iDisplayStart']);
			$limit = ' limit '.$iDisplayStart.','.$iDisplayLength;
			$iSortCol_0 = $oComments->setInt($_GET['iSortCol_0']);
			$sSortDir_0 = $oComments->setString($_GET['sSortDir_0']);
			if(!empty($columns[$iSortCol_0])){
