...
class News extends Database {
	...
	public function insertData($category_id,$name,$slug,$description,$content,$image,$params,$meta_key,$meta_description,$user_id,$status,$start,$end){
	...
	public function updateData($id,$category_id,$name,$slug,$description,$content,$image,$params,$meta_key,$meta_description,$user_id,$status,$start,$end){
		$this->sql ="update $this->table set category_id=$category_id,name='$name',slug='$slug',description='$description',content='$content',image='$image',params='$params',meta_key='$meta_key',meta_description='$meta_description',user_id=$user_id,status=$status,mdate=NOW(),start='$start',end='$end' where $this->primary_key = $id ";
		$this->update();
	}
	public function duplicateData($id,$user_id){
		...
		$this->select();
		$data = $this->rows[0] ;
		$this->insertData($data['category_id'],$data['name'],$data['slug'],$data['description'],$data['content'],$data['image'],$data['params'],$data['meta_key'],$data['meta_description'],$user_id,$data['status'],$data['start'],$data['end']) ;
		// insert translate 
	...
	public function setUpdateStatViewd($id) {
		$this->sql = "select id , stat from $this->table where $this->primary_key = $id ";
		$this->select();
		$rows = $this->rows[0];
		if(!empty($rows)) {
			$rows['stat'] += 1;
			$this->sql="update $this->table set stat = ".$rows['stat']." where $this->primary_key = $id ";
			$this->update();
		} else {
