...
///////////////////////////////////////////////////////////////////////////////////
	...
	function insertData($category_id,$name,$slug,$content,$image,$params,$link,$meta_key,$meta_description,$user_id,$status,$start,$end){
		$this->sql ="insert into $this->table (category_id,name,slug,content,image,params,link,meta_key,meta_description,user_id,status,mdate,cdate,start,end,sequence) ";
		$this->sql .=" values($category_id,'$name','$slug','$content','$image','$params','$link','$meta_key','$meta_description',$user_id,$status,NOW(),NOW(),'$start','$end',0) ";
		$this->insert();
	}
	function updateData($id,$category_id,$name,$slug,$content,$image,$params,$link,$meta_key,$meta_description,$user_id,$status,$start,$end){
		$this->sql ="update $this->table set category_id=$category_id,name='$name',slug='$slug',content='$content',image='$image',params='$params',link='$link',meta_key='$meta_key',meta_description='$meta_description',user_id=$user_id,status=$status,mdate=NOW(),start='$start',end='$end' where $this->primary_key = $id ";
		$this->update();
	}
	function duplicateData($id,$user_id){
		...
		$this->select();
		$data = $this->rows[0] ;
		$this->insertData($data['category_id'],$data['name'],$data['slug'],$data['content'],$data['image'],$data['params'],$data['link'],$data['meta_key'],$data['meta_description'],$user_id,$data['status'],$data['start'],$data['end']) ;
		// insert translate 
...
// page translate
  ...
  function saveTranslate( $lang,$id,$name,$content,$image,$params,$meta_key,$meta_description){
		...
		$chk = (empty($this->rows[0]['id']))?true:false ;
		if($chk){
			$this->sql ="insert into  $this->table_translate (lang, id, name, content ,image,params,meta_key,meta_description) values ('$lang',$id,'$name','$content','$image','$params','$meta_key','$meta_description')  ";
			$this->insert();
		}else{
			$this->sql = "update  $this->table_translate set lang='$lang', name='$name', content='$content',image='$image' ,params ='$params' ,meta_key='$meta_key', meta_description='$meta_description' where id=$id  ";
			$this->update();
