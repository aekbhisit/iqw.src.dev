...
class Modules extends Database {
	...
	public function insertData($category_id,$name,$slug,$content,$params,$javascirpt,$css,$meta_key,$meta_description,$user_id,$status){
	...
	public function updateData($id,$category_id,$name,$slug,$content,$params,$javascirpt,$css,$meta_key,$meta_description,$user_id,$status){
		$this->sql ="update $this->table set category_id=$category_id,name='$name',slug='$slug',content='$content',params='$params',javascript='$javascirpt',css='$css',meta_key='$meta_key',meta_description='$meta_description',user_id=$user_id,status=$status,mdate=NOW() where $this->primary_key=$id ";
		$this->update();
	}
