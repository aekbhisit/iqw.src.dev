...
// page translate
  ...
  function saveTranslate( $lang,$id,$name,$content,$image,$image1,$image2,$image3,$image4,$params,$meta_key,$meta_description){
		$this->sql ="select id from $this->table_translate where lang='$lang' and id=$id ";
		$this->select(); 
		$chk = (empty($this->rows[0]['id']))?true:false ;
		if($chk){
			$this->sql ="insert into  $this->table_translate (lang, id, name, content, params,meta_key,meta_description) values ('$lang',$id,'$name','$content','$image','$image1','$image2','$image3','$image4','$params','$meta_key','$meta_description')  ";
			$this->insert();
		}else{
