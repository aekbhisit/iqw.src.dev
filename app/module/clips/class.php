...
///////////////////////////////////////////////////////////////////////////////////
	...
	function duplicateData($id,$user_id){
		...
		if(!empty($data)){
			foreach($data as $d){ 
				$this->saveTranslate( $d['lang'],$new_id,$d['name'],$d['url'],$d['content'],$d['image'],$d['params'],$d['meta_key'],$d['meta_description']);
			}
		}
	}
...
// page translate
  ...
  function saveTranslate($lang,$id,$name,$url,$content,$params,$meta_key,$meta_description){
		$this->sql ="select id from $this->table_translate where lang='$lang' and id=$id ";
		$this->select(); 
		$chk = (empty($this->rows[0]['id']))?true:false ;
		if($chk){
			$this->sql ="insert into  $this->table_translate (lang, id, name, url, content ,params,meta_key,meta_description) values ('$lang',$id,'$name','$url','$content','$params','$meta_key','$meta_description')  ";
			$this->insert();
		}else{
			$this->sql = "update  $this->table_translate set lang='$lang', name='$name',url='$url', content='$content' ,params ='$params' ,meta_key='$meta_key', meta_description='$meta_description' where id=$id  ";
			$this->update();
