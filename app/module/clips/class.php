...
class Clips extends Database {
	...
	public function insertData($category_id,$name,$slug,$description,$url,$content,$params,$image,$meta_key,$meta_description,$user_id,$status){
	...
	public function updateData($id,$category_id,$name,$slug,$description,$url,$content,$params,$image,$meta_key,$meta_description,$user_id,$status){
	...
	public function duplicateData($id,$user_id){
		...
		if(!empty($data)){
			foreach($data as $d){ 
				$this->saveTranslate( $d['lang'],$new_id,$d['name'],$d['description'],$d['url'],$d['content'],$d['image'],$d['params'],$d['meta_key'],$d['meta_description']);
			}
		}
	}
	...
	public function getTranslate($id,$lang){
  	...
  	public function saveTranslate($lang,$id,$name,$description,$url,$content,$image,$params,$meta_key,$meta_description){
		$this->sql = "select id from $this->translate_table where lang='$lang' and id=$id ";
		$this->select(); 
		$chk = (empty($this->rows[0]['id']))?true:false;
		if($chk){
			$this->sql ="insert into $this->translate_table (lang,id,name,description,url,content,image,params,meta_key,meta_description) values ('$lang',$id,'$name','$description','$url','$content','$image','$params','$meta_key','$meta_description') ";
			$this->insert();
		}else{
			$this->sql = "update $this->translate_table set lang='$lang',name='$name',description='$description',url='$url',content='$content',image='$image',params ='$params',meta_key='$meta_key',meta_description='$meta_description' where id=$id ";
			$this->update();
		}
	}
