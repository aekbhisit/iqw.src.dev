...
///////////////////////////////////////////////////////////////////////////////////
	...
	function insertData($category_id,$name,$slug,$rating,$number_of_room,$address,$city,$postcode,$tel,$fax,$email,$website,$nearest_airport,$distance_to_airport,$hotel_location,$best_way_to_airport, $room_detail, $dinner, $rate_include,$children_policy,$facilities,$amenities,$contact_information,$user_id,$status){
		$this->sql ="insert into $this->table (category_id,name,slug,rating,number_of_room,address, city,postcode,tel,fax,email,website,nearest_airport,distance_to_airport,hotel_location,best_way_to_airport, room_detail, dinner, rate_include,children_policy,facilities,amenities,contact_information,user_id,status,mdate,cdate,sequence) ";
		$this->sql .=" values($category_id,'$name','$slug','$rating',$number_of_room,'$address','$city','$postcode','$tel', '$fax', '$email','$website','$nearest_airport','$distance_to_airport','$hotel_location','$best_way_to_airport', '$room_detail', '$dinner', '$rate_include','$children_policy','$facilities','$amenities','$contact_information', $user_id,$status,NOW(),NOW(),0) " ;
		echo $this->sql  ;
		$this->insert();
	}
	
	function updateData($id,$category_id,$name,$slug,$rating,$number_of_room,$address,$city,$postcode,$tel,$fax,$email,$website,$nearest_airport,$distance_to_airport,$hotel_location,$best_way_to_airport, $room_detail, $dinner, $rate_include,$children_policy,$facilities,$amenities,$contact_information,$user_id,$status){
		$this->sql ="update $this->table set category_id=$category_id, name='$name', slug='$slug',rating='$rating',number_of_room=$number_of_room,address='$address',city='$city',$postcode,tel='$tel',fax='$fax', email='$email', website='$website', nearest_airport='$nearest_airport',distance_to_airport='$distance_to_airport' ,hotel_location='$hotel_location', best_way_to_airport='$best_way_to_airport', room_detail='$room_detail', dinner='$dinner', rate_include='$rate_include', children_policy='$children_policy', facilities='$facilities' ,amenities='$amenities', contact_information='$contact_information', user_id=$user_id, status=$status, mdate=NOW() where $this->primary_key = $id ";
		echo $this->sql  ;
		$this->update() ;
	}
...
// page translate
  ...
  function saveTranslate( $lang,$id,$name,$content,$params,$meta_key,$meta_description){
		...
		if($chk){
			...
			$this->sql = "update  $this->table_translate set lang='$lang', name='$name', content='$content' ,params ='$params' ,meta_key='$meta_key', meta_description='$meta_description' where id=$id  ";
			$this->update();
		}
	}
