...
$params_category = array(
	'module'=>'rooms_categories',
 	'table'=>'rooms_categories',
	'table_translate'=>'rooms_categories_translate',
	'site_language'=>SITE_LANGUAGE,
	'is_translate'=>SITE_TRANSLATE
);
$oCategories = new Rooms($params_category);
$params = array(
	'module'=>'rooms',
 	'table'=>'rooms',
	'parent_table'=> 'rooms_categories',
	'parent_table_translate'=> 'roomss_categories_translate',
	'parent_primary_key'=> 'id',
	'table_translate'=>'rooms_translate',
	'site_language'=>SITE_LANGUAGE,
	'is_translate'=>SITE_TRANSLATE 
);
$oModule = new Rooms($params);
if(isset($_GET['task'])){
	$task =$_GET['task'] ;
	switch($task){
		//  categories task   
			...
			if(!empty($categories)){
				foreach($categories as $key =>$value){
					if($value['level']>0){
						...
						$output["aaData"][] = array(0=>$row_chk,1=>$indent.$value['name'],2=>$value['level'],3=>$value['mdate'],4=>$order,5=>$iconbar,6=>$value['id'] ,"DT_RowClass"=>'row-'.$cnt,"DT_RowId"=>$value['id']);
						$cnt++;
					}
				}
			}
		...
		case 'getData':
			...
			if(!empty($data)){
				foreach($data as $key =>$value){
					...
					$output["aaData"][] = array(0=>$row_chk,1=>$indent.$value['name'],2=>$value['category'],3=>$value['mdate'],4=>$order,5=>$iconbar,6=>$value['id'] ,"DT_RowClass"=>'row-'.$cnt,"DT_RowId"=>$value['id']);
					$cnt++;
				}
			}
			echo json_encode($output);
		...
		case 'saveData':
			...
			$concept = $oModule->setString($_POST['concept']);
			$room_description = $oModule->setString($_POST['description']);
			...
			foreach($new_sort_list as $nk=>$nv){
				...
				$sorted_arr[(int)$nk]=array('id'=>$nk,'src'=>$img_arr[(int)$nv],'title'=>$title,'description'=>$description);
			}
			ksort($sorted_arr);
			$gallery = json_encode($sorted_arr);
			// end gallery images
			$facility = json_encode($_POST['facility']);
			$price = (!empty($_POST['price']))?$_POST['price']:0;
			$discount = (!empty($_POST['discount']))?$_POST['discount']:0; //$_POST['discount'];
			$currency = $oModule->setString($_POST['currency']);
			//metadata  
			...
			$params = '';
			if(empty($id)){		
				$oModule->insertData($category_id,$name,$slug,$concept,$room_description,$content,$image,$gallery,$facility,$price,$discount,$currency,$params,$meta_key,$meta_description,$user['id'],$status);
			}else{
				$oModule->updateData($id,$category_id,$name,$slug,$concept,$room_description,$content,$image,$gallery,$facility,$price,$discount,$currency,$params,$meta_key,$meta_description,$user['id'],$status);
			}
		break;
		case 'setStatus':
			$id = $oModule->setInt($_GET['id']);
		...
		case 'saveTranslate':
			$id = $oModule->setInt($_POST['id']);
			$lang = $oModule->setString($_POST['language']);
			$name = $oModule->setString($_POST['name']);
			$content = $oModule->setString($_POST['content']);
			$content1 = $oModule->setString($_POST['content1']);
			$content2 = $oModule->setString($_POST['content2']);
			$meta_key = $oModule->setString($_POST['meta_key']);
			$meta_description = $_POST['meta_description'];
			$oModule->saveTranslate($lang,$id,$name,$content,$content1,$content2,'',$meta_key,$meta_description);
		break;
		case 'getDataInFinds':
			$columns = array('id','name','category_id','mdate','sequence');
			$limit = '';
