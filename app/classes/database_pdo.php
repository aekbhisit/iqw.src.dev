...
class Database {
	...
	public function connect(){
		...
		//$this->conn = new PDO("mysql:dbname={$dbname};host={$host}", $user, $pass);
		//print_r($this->conn);
		//new PDO("mysql:dbname={DB_DATABASE};host={DB_SERVER}", DB_USER, DB_PASS); //mysqli_connect(DB_SERVER,DB_USER,DB_PASS); //new 
		...
		// $result = $mysqli->query("select * from stat") ;
		// var_dump($result) ;
		// if ($result) {
	...
	public function select_db(){
		//mysqli_select_db(DB_DATABASE,$this->conn);
	...
	public function query(){
		...
		//echo $this->sql;
		//$this->result = mysqli_query($this->sql) or die( mysqli_error() );
		//print_r($this->result) ;
	}
	...
	public function fields_name($offset) {
		return mysqli_field_name($this->result);
	}
	...
	function _valid($inputVar, $type='text') {      
      switch ($type) {
		...
		//	echo '908'.$inputVar;
        	...
        	return addslashes(htmlentities($inputVar, ENT_QUOTES)); 
        } 
	}
